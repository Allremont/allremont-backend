...
class PlintusSpider(scrapy.Spider):
    name = "plintusy"

    start_urls = [
        'https://megastroy.kz/astana/market/napolnye-pokrytiya/plintusy/'
    ]
    max_pages = 100
    rules = (Rule(LinkExtractor(allow=('\\search?page=\\d')), 'parse_start_url', follow=True),)

    custom_settings = {
        'FEED_URI': 'dataset/plintusy.csv'
    }

    def start_requests(self):
        for i in range(self.max_pages):
            yield scrapy.Request('https://megastroy.kz/astana/market/napolnye-pokrytiya/plintusy/?page=%d' % i,
                                 callback=self.parse)

    def parse(self, response):
        response.selector.remove_namespaces()

        # Extract plintusy information
