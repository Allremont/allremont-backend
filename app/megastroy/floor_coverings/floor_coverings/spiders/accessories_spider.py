...
class AccessoriesSpider(scrapy.Spider):
    name = "accessories"

    start_urls = [
        'https://megastroy.kz/astana/market/napolnye-pokrytiya/akssesuary-dlya-plintusov/'
    ]
    max_pages = 100
    rules = (Rule(LinkExtractor(allow=('\\search?page=\\d')), 'parse_start_url', follow=True),)

    custom_settings = {
        'FEED_URI': 'dataset/accessories.csv'
    }

    def start_requests(self):
        for i in range(self.max_pages):
            yield scrapy.Request(
                'https://megastroy.kz/astana/market/napolnye-pokrytiya/akssesuary-dlya-plintusov/?page=%d' % i,
                callback=self.parse)

    def parse(self, response):
        response.selector.remove_namespaces()

        # Extract accessories information
